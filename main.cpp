...
int main()
{
    ...
    */
    ...
    while (p.check(showGrid) == false) {//判斷直橫區是否相加有45
        cout << "Did not enter all digits in 1~9.Please enter the complete 1~9 ";//未輸入0~9內所有數字 請輸入完整的0~9
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                if (showGrid[i][j] == 0) {
                    int input;
                    ...
                    if (p.nReaptingBlock(input, 9*i+j, showGrid) == false || p.nReaptingCroner(input, 9*i+j, showGrid) == true) {//判斷此數字是否重複
                        showGrid[i][j] == 0;
